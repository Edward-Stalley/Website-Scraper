...
export default function Home() {
  ...
  const [savedArticles, setSavedArticles] = useState<ReactElement<any, any>[]>([]);
  ...
  return (
      ...
      <div className="bg-slate-800 justify-center items-center p-20 flex flex-col">
        ...
        <div className=" flex-col flex gap-6 justify-center items-center p-2">
          ...
          <button
            ...
            className="rounded-xl p-5 sm:col-span-2  bg-slate-300 text-slate-700"
          >
            {!clicked ? <p>Show Bookmarks</p> : <p>&#9733;</p>}
          </button>

          {clicked && (
            <div className="flex sm:w-full w-64 max-w-3xl justify-center items-center py-1 mt-20">
              <div className="w-full h-full max-w-3xl">
                <Carousel items={savedArticles} />
              </div>
            </div>
          )}
        </div>
        {/* need to fix this logic */}
        {/* showing loading screen when bookmarks are clicked - need to fix */}
        {!loading && !clicked ? (
          <div className="flex sm:w-full w-64 max-w-3xl justify-center items-center py-1 mt-20">
            <div className="w-full h-full max-w-3xl">
    ...
    w-8 h-8 mr-2 mt-20 text-gray-200 animate-spin dark:text-gray-600 `}
              viewBox="0 0 100 101"
              fill="none"
